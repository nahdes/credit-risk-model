...
python -m src.train --data_path data/raw/transactions.csv
...
python -m src.predict --input data/new_user.json

# Batch predictions
python -m src.predict --input data/new_users.csv --batch --output predictions.csv
```
...
curl -X POST http://localhost:8000/predict/batch \
  ...
  -d '{
    "customers": [
      {"customer_id": "CUST_001", "recency_days": 10, ...},
      {"customer_id": "CUST_002", "recency_days": 25, ...}
    ]
  }'
```
